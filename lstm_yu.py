...
# 从 Excel 文件中读取数据
data_df = pd.read_excel(r"./data/2019_08_min.xlsx", sheet_name="Sheet1",engine='openpyxl')
...
# selected_features = ['AH','AT', 'GHI','GTI','MT','pac']
target_column = 'Pdc'
...
# data_normalized2 = (datay - datay_mean) / datay_std #(16638,1)
# #data_normalized1和data_normalized2是所有的数据

data_num = len(data_normalized1) #总样本数量
train_num = int(data_num*0.8)  #训练集样本数量

X = data_normalized1[0:train_num,:]
y = data_normalized2[predict_min:predict_min+train_num,-1]
data_time = data_time[0:train_num]

# 划分特征和目标变量
# X = data_normalized1[:, :]
# y = data_normalized2[:, -1]
...
# loaded_model = Net(input_size, hidden_size, num_layers, output_size)
...
# 指定输出文件名
output_file = "./model/lstm_2input_pdc_shift_xlp.onnx"
